...
def scrabble(word):
    ...
    points.update(dict.fromkeys('авеёинорст', 1))
    points.update(dict.fromkeys('дклмпу', 2))
    points.update(dict.fromkeys('бгья', 3))
    points.update(dict.fromkeys('йы', 4))
    points.update(dict.fromkeys('жзхцч', 5))
    points.update(dict.fromkeys('фшэю', 8))
    points.update(dict.fromkeys('щ', 10))
    points.update(dict.fromkeys('ъ', 15))

    points = (sum(points[letter] for letter in word))
    return points
